...
class GoogleDriveRoutes {
	...
	private const AUTHORIZE_ROUTE = '/gdrive/authorize';
	private const CALLBACK_ROUTE  = '/gdrive/callback';
	private const STATE_PREFIX    = 'msjfb_gdrive_state_';
	...
	public function prepare_authorization( WP_REST_Request $request ) {
		$settings      = SettingsRepository::provider( 'gdrive' );
		$client_id     = trim( $settings['client_id'] ?? '' );
		$client_secret = trim( $settings['client_secret'] ?? '' );

		if ( '' === $client_id || '' === $client_secret ) {
			return new WP_Error(
				'msjfb_gdrive_missing_credentials',
				__( 'Enter the Google Client ID and Secret before generating tokens.', 'media-storage-for-jetformbuilder' ),
				array( 'status' => 400 )
			);
		}
		...
		$this->log( 'Prepared Google authorization URL.', array( 'callback' => $callback ) );
	...
	public function handle_callback( WP_REST_Request $request ) {
		...
		if ( '' !== $error ) {
			$this->log( 'Google OAuth returned error.', array( 'error' => $error ) );
			return $this->render_message(
				array(
					'status'  => 'error',
					'message' => sprintf( __( 'Google reported an error: %s', 'media-storage-for-jetformbuilder' ), $error ),
				)
			);
		}
		...
		if ( ! $state_payload ) {
			$this->log( 'Google state expired/missing.', array( 'state' => $state ) );
			return $this->render_message(
				array(
					'status'  => 'error',
		...
		$this->log( 'Google Drive tokens saved via callback.' );

		return $this->render_message(
			array(
				'status'  => 'success',
				'tokens'  => $result,
				'message' => __( 'Google Drive tokens saved. You can close this window.', 'media-storage-for-jetformbuilder' ),
			)
		);
	}
	...
	private function exchange_code_for_tokens( string $code ) {
		...
		if ( '' === $client_id || '' === $client_secret ) {
			return new WP_Error(
				'msjfb_gdrive_missing_credentials',
				__( 'Google Client ID or Secret is missing.', 'media-storage-for-jetformbuilder' )
			);
		}
		...
		if ( 200 !== $http_code || ! is_array( $data ) || empty( $data['access_token'] ) ) {
			$this->log( 'Token endpoint returned unexpected response.', array( 'code' => $http_code, 'error' => $data['error_description'] ?? $data['error'] ?? 'unknown' ) );
			return new WP_Error(
				'msjfb_gdrive_invalid_response',
				__( 'Google did not return the expected tokens. Try again later.', 'media-storage-for-jetformbuilder' )
			);
		}
	...
	private function render_message( array $payload ) {
		$json_payload = wp_json_encode(
			array_merge( array( 'source' => 'msjfb-gdrive' ), $payload )
		);
		...
		$html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Google Drive OAuth</title>'
	...
	private function log( string $message, array $context = array() ): void {
		...
		error_log( sprintf( '[MSJFB GDrive] %s %s', $message, $payload ) );
	}
}
